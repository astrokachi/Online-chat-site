...
export const Profile = () => {
  ...
  // }, [imgs])
  return (
    <div className="bg-gradient-to-b min-h-screen text-white p-4 from-start text-center to-black">
      <Nav />
      <main className="bg-purp bg-opacity-10 h-max w-[90%] mb-8 mt-12 p-4 pb-24 mx-auto">
        ...
        <div>
          ...
          <section className="flex justify-center gap-3 px-6  pt-8 max-w-full flex-wrap ">
            {[...Array(6)].map((_, index) => {
              return (
                <div key={index} className={`${user && !user[`images${index}`].includes('undefined') ? 'md:w-[35%] w-[47%]' : 'md:h-[150px] md:w-[25%] w-[47%] h-[100px]' }  ${index} rounded bg-gray-400 relative`}>
                  {user && !user[`images${index}`].includes('undefined') && <img src={user[`images${index}`]} alt='' className='h-full w-full rounded'/>}
                  <div>
                    <label htmlFor="photo">
                      <img
          ...
          <button className="px-8 py-2 mt-10 mx-auto text-sm bg-purp bg-opacity-70 rounded-lg" onClick={updates}>
            Save
          </button>
        </div>
      </main>
